...
def check_source_entry(source: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    if source_keys[-1] != "helm":
        ...
        orghelm = source.get("helm")
        source.pop("helm", None)
        source["helm"] = orghelm
        retval = False
    ...
    if move_values:
        ...
        for keyname in ["values", "valuesObject"]:
            if keyname in helm_keys:
                orgval = helm_section[keyname]
                helm_section.pop(keyname, None)
                helm_section[keyname] = orgval
    ...
    if spec_keys[-1] not in ["source", "sources"]:
        ...
        for keyname in ["source", "sources"]:
            if keyname in spec_keys:
                orgval = manifest.get("spec").get(keyname)
                manifest.get("spec").pop(keyname, None)
                manifest["spec"][keyname] = orgval
